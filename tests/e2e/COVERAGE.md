...
| --- | --- | --- |
| Private runtime setup | Dry-run, real isolated installation of the pinned storage runtime, executable verification, and idempotent reuse | Failed replacement rollback and concurrent setup lock |
| Repository initialization | Dry-run, full scaffold, unmanaged `AGENTS.md` refusal with no partial files, idempotency, managed-config drift detection and repair | Partial engine failure rollback, pre-existing unmanaged internal config, removal of unused S3 configuration, unsafe URLs, and incompatible CLI constraints |
| Effective policy | Every enabled instruction topic, disabled-topic rejection, model-before-rules ordering, repository module composition, human and JSON output, and stable policy hashes | Module-selection permutations and documentation vocabulary contract |
| Configuration and diagnostics | Public configuration hides the private engine; review ownership is resolved; doctor passes with versioned S3 and fails when versioning or managed config is unhealthy | Schema, URL, release-build filesystem-remote, and command-version validation |
| Deliverable task scaffold | Dry-run, creation, duplicate refusal without clobbering, manifest discovery from the task and explicit path, branch initialization, README, and shared checkout staying on `main` | Invalid slug/timestamp/branch and missing-README publication guard |
...
| Infrastructure task scaffold | Dry-run, private manifest, isolated worktree, no task directory, exact declared scopes, undeclared-change refusal, shared private cache/config links, Git plus S3 publication, and a clean worktree | Invalid/missing infrastructure scopes and explicit-manifest lookup from another checkout |
| Scope authorization | Default task-only publication, unrelated overlay exclusion, one-shot root scope requiring a reason, authorization trailer, and infrastructure out-of-scope refusal | Symlink traversal, nested Git checkout/gitlink, multiple include normalization, and path parser permutations |
