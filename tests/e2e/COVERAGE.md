...
| Task slug rename | Published deliverable dry-run and apply, stable task/branch/review identity, complete directory and S3-metadata move, no remote writes before publication, old-tree deletion plus new-tree publication on the same network branch, published-placement preservation, path-bound S3 version rematerialization, exact hydration from the renamed S3 path, and final clean plan | Unpublished and legacy-schema rename, invalid/same/colliding/staged/merged refusal, large published-Git placement preservation, cloud-version reset, infrastructure metadata rename, and rollback guards |
...
| Task discard | Published deliverable and infrastructure dry-runs, explicit agent review handoff, exact-ID confirmation from the shared checkout, leased network-branch deletion, local branch/directory/worktree/private-state cleanup, and S3-version retention | Missing/stale confirmation plans, changed refs, merged-task refusal, additional-scope/index restoration, wrong-ID/current-directory guards, remote rejection rollback, and pointer-version parsing |
