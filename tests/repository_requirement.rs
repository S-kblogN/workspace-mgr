...
/// `init` owns two things side by side: the generated root `.gitignore`, which
/// it reconciles, and the configuration's `minimum_cli_version`, which it keeps
/// exactly. `doctor` reports drift in the one beside the requirement of the
/// other, and a repository that requires a newer release is diagnosed, never
/// repaired, by a release that does not meet it.
#[test]
fn init_reconciles_the_root_ignore_file_and_keeps_the_declaration() {
    ...
    let generated = read(&ignore_path);
    assert!(generated.starts_with("# Generated by workspace-mgr."));
    let declared = declaring("0.2.0", &plain);
    let drifted = format!("{generated}# a hand edit below the generated header\n");
    std::fs::write(&config_path, &declared).unwrap();
    std::fs::write(&ignore_path, &drifted).unwrap();

    let report = json(&workspace_unchecked(&fixture.seed, ["doctor"]));
    assert_eq!(failing_checks(&report), ["repository-scaffold"]);
    let scaffold = doctor_check(&report, "repository-scaffold");
    assert!(
        scaffold["detail"].as_str().unwrap().contains(".gitignore"),
        "{scaffold}"
    );
    assert_eq!(
        doctor_check(&report, "cli-version")["detail"],
    ...
    let actions = repaired["actions"]
        ...
        .collect::<Vec<_>>();
    assert_eq!(actions, [("update".to_owned(), ".gitignore".to_owned())]);
    assert_eq!(read(&ignore_path), generated);
    assert_eq!(read(&config_path), declared);
    assert_eq!(json(&workspace(&fixture.seed, ["doctor"]))["status"], "ok");

    // A declaration this release does not meet stops `init` before it touches
    // either file, while `doctor` still reports both.
    let required = declaring("99.0.0", &plain);
    ...
    assert_eq!(read(&config_path), required);
    let report = json(&workspace_unchecked(&fixture.seed, ["doctor"]));
    assert_eq!(
        failing_checks(&report),
        ["cli-version", "repository-scaffold"]
    );
}
